 ...
 */
double controller(double current, double target, struct control_t &pid) {
  // TODO: Investigate using constant sample time instead of current time for
  // simplicity
...
double motor_speed(double velA, double velB, double distance, bool &done) {
  ...
  double nextX = lastX + SAMPLE_TIME / 1000.0 * wheel_size * cos(phi) *
                             (velA + velB) / 2.0;
  double nextY = lastY + SAMPLE_TIME / 1000.0 * wheel_size * sin(phi) *
                             (velA + velB) / 2.0;
...
//  double my_phi_dot = wheel_size*(velA - velB)/wheel_dist; // wheel_size,
//  wheel_dist
//
...
double motor_direction(double velA, double velB, double turning, bool &done) {
  ...
  double phi_dot =
      wheel_size * (velA - velB) / wheel_dist; // wheel_size, wheel_dist
...
bool motor_control(int &command, double target_distance, double target_angle) {
  ...
  // double nextX = lastX + SAMPLE_TIME/1000.0 * wheel_size* cos(phiOld)*
  // (velA+velB)/2.0; double nextY = lastY + SAMPLE_TIME/1000.0 * wheel_size*
  // sin(phiOld)* (velA+velB)/2.0;
  // double current_angle = read_angle();
  //  //double newPhi = phiOld + SAMPLE_TIME/1000.0 *
  //  (wheel_size/wheel_dist)*(velA-velB);
